...
- **Media reload:** `VideoSurface::mark_media_changed()` on every `PlaybackEngine::load` clears `overlay_bound` and detaches stale `AVSampleBufferDisplayLayer` sublayers from the host `UIView` (URI→URI reload, not only shell kind change).
...
- **Verified CALayer attach:** `xhvp_ios_attach_layer_to_host_sync` returns `bool` — host bounds must be non-zero and sublayer must appear in hierarchy. `read_sink_layer` `CFRetain`s; shim `CFRelease`s only on verified attach; Rust `release_sink_layer` on defer/failure. **Preferred order: read `layer` at READY → sync attach on main → PAUSED preroll → PLAYING.** Fallback when `layer` unreadable at READY: PAUSED preroll → read `layer` → **immediate sync attach** in the same xhvp-gst turn (never async for first bind). **No PAUSED preroll** until `xhvp_ios_host_view_has_bounds` is true; failed attach rolls pipeline back to `READY` (skipped when attach lifecycle is stale). **`AVSampleBufferDisplayLayer`** gets `videoGravity = resizeAspect` and `flushAndRemoveImage` before add/re-add; `xhvp_ios_detach_sink_layers_from_host` clears stale sublayers on media/host change.
...
- **`IosOverlaySession`** (`playback/ios_overlay.rs`): single seam for iOS overlay attach phase — `request_attach` dedupes via `attach_in_flight`, `finish_attach` sets `overlay_bound` only after verified sync attach, `schedule_apply` / `schedule_attach` coalesce idle work, and `apply_target_state` implements Tutorial 4 `target_state` + Tutorial 12 buffering (`buffering_active`). Bus callbacks set flags only — zero `set_state_sync` / inline attach on iOS. **`overlay_generation`** + **`running`** invalidate queued idle work and attach completions (`IosAttachLifecycle`) on `load`, shell rebuild, and `PlaybackEngine::Drop`; paired with `running` to match bus-watch teardown semantics.
