...
/// C ABI frame bridge (`xhvp_texture_*` in native/).
...
/// when a new frame is available. `ctx` is the unretained `XueHuaVideoTexture`.
...
/// Flutter external texture backed by GStreamer BGRA frames from the C core and
...
final class XueHuaVideoTexture: NSObject, FlutterTexture {
    ...
    // Reused staging buffer (native frame incl. row stride) and pixel buffer.
    ...
    /// Called from the native streaming thread. `textureFrameAvailable` is thread-safe.
