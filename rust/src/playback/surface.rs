...
impl VideoSurface {
    pub fn new(stored: std::sync::Arc<Mutex<Option<usize>>>) -> Self {
        Self {
            ...
            #[cfg(target_os = "ios")]
            last_applied_handle: Arc::new(AtomicUsize::new(0)),
            #[cfg(any(target_os = "macos", target_os = "ios"))]
            overlay_sink: None,
        }
    }
    ...
    /// True once `glimagesink` received `set_window_handle` on the Gst thread (Android/iOS).
    #[cfg(any(target_os = "android", target_os = "ios"))]
        ...
        #[cfg(any(target_os = "android", target_os = "ios"))]
        {
            if let Some(handle) = *self.stored.lock() {
                ...
                #[cfg(target_os = "ios")]
                {
                    self.overlay_bound.store(false, Ordering::SeqCst);
                    self.last_applied_handle.store(0, Ordering::SeqCst);
                    if let Some(slot) = self.overlay_sink.as_ref() {
                        let sink = slot.lock().clone();
                        if let Err(e) =
                            crate::video::ios_layer::attach_ios_video_layer(shell, &sink, handle)
                        {
                            log::warn!("gst: ios layer rebind: {e:#}");
                        } else {
                            self.overlay_bound.store(true, Ordering::SeqCst);
                            self.last_applied_handle.store(handle, Ordering::SeqCst);
                            log::info!("gst: ios layer rebound on new video_sink");
                        }
                    }
                }
            }
    ...
    pub fn cache_ios_overlay(&self, handle: i64, width: i32, height: i32) {
        ...
        self.cache_handle(handle as usize);
        self.overlay_bound.store(false, Ordering::SeqCst);
        self.last_applied_handle.store(0, Ordering::SeqCst);
    }
    ...
    pub fn apply_ios_overlay_gstreamer(
        ...
        if self.overlay_bound.load(Ordering::SeqCst) && self.last_applied_handle.load(Ordering::SeqCst) == host_view
        {
            return Ok(());
        }
        ...
        let last_applied = self.last_applied_handle.clone();
        spawn_on_gst_thread(move || {
            let mut guard = shell.lock();
            let Some(slot) = overlay_sink.as_ref() else {
                return;
            };
            let sink = slot.lock().clone();
            match crate::video::ios_layer::attach_ios_video_layer(&guard, &sink, host_view) {
                Ok(()) => {
                    overlay_bound.store(true, Ordering::SeqCst);
                    last_applied.store(host_view, Ordering::SeqCst);
                }
                Err(e) => {
                    overlay_bound.store(false, Ordering::SeqCst);
                    log::warn!("ios layer attach: {e:#}");
                    return;
                }
            }
            if let Err(e) = maybe_preroll_and_resume_play(&mut guard, &play_intent, host_view, width, height)
            {
                log::warn!("ios preroll after layer attach: {e:#}");
            }
        });
        Ok(())
    }
...
pub fn apply_mobile_overlay_on_gst(
    ...
    #[cfg(target_os = "ios")]
    {
        let last_applied = last_applied_handle
            .ok_or_else(|| anyhow!("ios overlay bind requires last_applied_handle"))?;
        let Some(slot) = play_intent.switch_ctx.surface.overlay_sink_slot() else {
            return Err(anyhow!("ios overlay sink slot missing"));
        };
        let sink = slot.lock().clone();
        crate::video::ios_layer::attach_ios_video_layer(shell, &sink, handle)?;
        last_applied.store(handle, Ordering::SeqCst);
    }
    overlay_bound.store(true, Ordering::SeqCst);
