...
pub fn attach_java_vm() -> Result<()> {
    ...
    // SAFETY: pointer came from `JavaVM::as_raw` in a JNI callback.
    let vm = unsafe { jni::JavaVM::from_raw(vm_ptr)? };
    vm.attach_current_thread_as_daemon(|_| Ok(()))?;
    Ok(())
}

/// Converts a Java `android.view.Surface` to a native window handle for VideoOverlay.
#[cfg(target_os = "android")]
pub fn native_window_handle_from_surface(env: &mut JNIEnv, surface: JObject) -> Result<usize> {
