...
impl IOSurface {
    ...
    /// Uploads tightly/loosely packed pixel `data` whose rows are `src_stride`
    /// bytes apart into the surface, honoring the surface's own (possibly larger,
    /// alignment-padded) row stride by copying one row at a time.
    pub fn upload(&self, data: &[u8], src_stride: usize) {
        unsafe {
