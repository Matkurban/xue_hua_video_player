...
默认会以**预编译二进制**方式获取 Rust 核心，因此作为插件使用方，通常**无需**安装 Rust
工具链；即使机器上已经装了 Rust 也是如此（详见[预编译二进制](#预编译二进制)）。桌面端与 iOS 仍需在构建/运行时提供 GStreamer
...
- 使用方默认走预编译二进制路径；即使机器上已经安装了 Rust / `rustup` 也是如此。
- 只有维护者在**明确需要**本地源码构建时，才需要在应用 `pubspec.yaml` 旁新建
...
- 如果当前 `crate-hash` 没有对应的已签名预编译产物，或下载因网络问题失败，cargokit 会回退到本地源码构建（v1.0.2+ 对下载失败自动重试最多 10 次）。
- 若要显式强制走预编译路径，也可以在应用 `pubspec.yaml` 旁新建
  `cargokit_options.yaml`：
...
macOS / iOS / Linux / Windows 构建时若出现如下错误：
...
说明 Cargokit 在从 GitHub Releases 下载预编译 Rust 库（macOS 静态库约 29 MB）时网络连接中断。**v1.0.2+** 已增强自动重试与失败回退；若仍失败，可按优先级尝试：
...
2. **改善 GitHub 访问**：确保能稳定访问 `release-assets.githubusercontent.com`；国内环境建议配置代理，避免公司防火墙拦截大文件 CDN。

3. **关闭预编译、本地编译**（需 Rust + GStreamer，适合维护者）：

   在应用 `pubspec.yaml` 同级创建 `cargokit_options.yaml`：

   ```yaml
   use_precompiled_binaries: false
   ```

   macOS 还需：`brew install gstreamer pkg-config`，以及 `rustup` 工具链。

4. **升级插件**：确保使用 `xue_hua_video_player` **v1.0.2** 或更高版本。
